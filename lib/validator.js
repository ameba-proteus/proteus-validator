...
(function(){
	...
	*/
	var FORMATS = {
		 ...
		 */
		'utc-millisec'
			: function(input) { return !_.isNaN(parseInt(input));},
...
//			: /^((?=.*::)(?!.*::.+::)(::)?([\dA-F]{1,4}:(:|\b)|){5}|([\dA-F]{1,4}:){6})((([\dA-F]{1,4}((?!\3)::|:\b|$))|(?!\2\3)){2}|(((2[0-4]|1\d|[1-9])?\d|25[0-5])\.?\b){4})$/i,
			...
			case TYPES.OBJECT:
				...
				for (var property in instance) {
					...
					if (!_.has(schema[PROPERTIES.PROPERTIES], property)) {
						var additionalProperties = schema[PROPERTIES.ADDITIONAL_PROPERTIES];
						// if additionalProperties is defined in schema
						if (!_.isUndefined(additionalProperties) && !_.isNull(additionalProperties)) {
							// ignore if addtionalProperties are 'true'
							if (_.isBoolean(additionalProperties) && additionalProperties) {
								continue;
	...
	*/
	Validator.prototype.validateTypeNumber = function(schema, instance, path) {
		if (!_.isNumber(instance) || _.isNaN(instance)) {
	...
	*/
	Validator.prototype.validateMinimum = function(schema, instance, path) {
		if (_.has(schema, PROPERTIES.MINIMUM)) {
			var minimum = schema[PROPERTIES.MINIMUM];
			var exclusiveMinimum = schema[PROPERTIES.EXCLUSIVE_MINIMUM];
			if (exclusiveMinimum) {
				if (!(minimum < instance)) {
	...
	*/
	Validator.prototype.validateMaximum = function(schema, instance, path) {
		if (_.has(schema, PROPERTIES.MAXIMUM)) {
			var maximum = schema[PROPERTIES.MAXIMUM];
			var exclusiveMaximum = schema[PROPERTIES.EXCLUSIVE_MAXIMUM];
			if (exclusiveMaximum) {
				if (!(instance < maximum)) {
