...
(function(){
	...
	*/
	var FORMATS = {
		 ...
		 */
		'utc-millisec'
			: function(input) { return !isNaN(parseInt(input));},
...
//			: /^((?=.*::)(?!.*::.+::)(::)?([\dA-F]{1,4}:(:|\b)|){5}|([\dA-F]{1,4}:){6})((([\dA-F]{1,4}((?!\3)::|:\b|$))|(?!\2\3)){2}|(((2[0-4]|1\d|[1-9])?\d|25[0-5])\.?\b){4})$/i,
	...
	// nextTick for browser compatible
	var nectTick;
	...
	Schema.prototype.validateObject = function(schema, path) {
		...
		switch(schema[TYPE]) {
			...
			case ARRAY:
				...
				break;
			default:
				self.error(path, TYPE, 'must be one of [' + [STRING,NUMBER,INTEGER,BOOLEAN,NULL,ANY,OBJECT,ARRAY].join(' | ') + ']', schema[TYPE]);
				break;
		}
	}
